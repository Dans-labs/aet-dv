...
function convertToGeojsonCoordinates(
  coordinatesArray: { x: number; y: number }[],
  type: string,
) {
...
export const maptilerApi = createApi({
  ...
  endpoints: (build) => ({
    fetchCoordinateSystems: build.query({
      ...
      transformResponse: (
        response: MaptilerCoordinateSystemResponse,
        _meta,
        arg,
      ) => {
                  ...
                  (item) =>
                    item.transformations && {
                      ...
                      bbox: [
                        ...
                        [item.bbox[2] + 0.2, item.bbox[3] + 0.2],
                      ],
                    },
                )
                .filter(Boolean),
    ...
    transformCoordinates: build.query({
      query: ({ type, coordinates, to, from }) => {
        const stringCoordinates =
          type !== undefined ? geojsonToString(type, coordinates) : coordinates;
        return {
